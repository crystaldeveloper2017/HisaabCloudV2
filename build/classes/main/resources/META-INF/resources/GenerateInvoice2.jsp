...
<c:set var="listUniqueModelNo" value='${requestScope["outputObject"].get("listUniqueModelNo")}' />
...
function saveInvoice()
{
	...
	// commented as of now will code in this later	
/* 	if(messageToShow!="")
		{
...
<datalist id="uniqueModelList">

<c:forEach items="${listUniqueModelNo}" var="modelno">
			    <option id="${customer.customerId}">${modelno.ModelNo}</option>			    
	   </c:forEach>	   	 
...
<div class="col-sm-3">
  ...
  <div class="input-group">
    <input type="text" class="form-control form-control-sm"   placeholder="model no"  list="uniqueModelList" id="model_no" name="model_no" oninput="checkforMatchItem()">    
  </div>
  </div>
    ...
    <div class="col-sm-2">
  <div class="input-group">
    <input type="text" class="form-control form-control-sm"   placeholder="unique no"  id="unique_no" name="unique_no" oninput="checkforMatchItem()">    
  </div>
  </div>
...
if('${invoiceDetails.invoice_id}'!='')
	{
		...
		document.getElementById("model_no").value="${invoiceDetails.model_no}";
		document.getElementById("unique_no").value="${invoiceDetails.unique_no}";
