	...
	<br>
              ...
              <div class="card-body table-responsive p-0" style="height: 800px;">                
                <table id="example1"class="table table-head-fixed  table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
                  ...
                  <tbody>
				<c:forEach items="${message}" var="data1">
					<tr >
					  
					  
					  <td>${data1.deposit_id}</td>
					  <td>${data1.account_no}</td>
             <td>${data1.bank_name}</td>
					  <td>${data1.txtdate}</td>
					  <td>${data1.updatedByuser}</td>
					  <td>${data1.updated_date}</td>
					
				<td><button class="btn btn-danger" onclick="deleteCashDepositRegister('${item.deposit_id}')">Delete</button></td>
...
<script >
  ...
  function exportSalesRegister2()
  {
	try
	{
		...
		window.open("?a=exportSalesRegister2&txtfromdate="+txtfromdate.value+"&txttodate="+txttodate.value);
		return;
  ...
  {	 	  
  	  		window.location="?a=showCashDepositRegister&txtfromdate="+txtfromdate.value+"&txttodate="+txttodate.value;
  ...
  function deleteCashDepositRegister(depositid)
{
	...
	var xhttp = new XMLHttpRequest();
	  ...
	  xhttp.open("GET","?a=deleteCashDepositRegister&depositid="+depositid, true);    
	  xhttp.send();
}
  
  document.getElementById("divTitle").innerHTML="Cash Deposit Register";
  document.title +=" Cash Deposit Register ";
