...
function saveInvoice()
{
	...
	var reqString="itemDetails="+itemString;
	  ...
	  xhttp.open("POST", "?a=saveRMStock", true);
...
<c:forEach items="${RawMaterialsList}" var="item">
  ...
  <div class="col-sm-12">  
   ...
   <div class="col-sm-12">
  	 <div class="form-group" align="center">	  
	 	...
	 	Total Raw Material : <span id="totalQty" style="font-weight:800">0</span>
...
function getRawMaterialDetailsAndAddToTable(itemId,itemName,qty)
{
...
document.getElementById("divTitle").innerHTML=" CURRENT RM STOCK ";
document.title +=" CURRENT RM STOCK ";
...
if('${invoiceDetails.invoice_id}'!='')
	{
		...
		var m=0;
		var tableNo="";
		<c:forEach items="${invoiceDetails.listOfRawMaterials}" var="item">
...
function getRawMaterialsAndStockForThisDate()
{

	


$('#tblrawmaterials tr').not(':first').remove();

  $.get("?a=getRawMaterialsAndStockForThisDate", function(data, status){

	var listOfRawMaterials=JSON.parse(data);

	for(var k=0;k<listOfRawMaterials.length;k++)
	{		
		getRawMaterialDetailsAndAddToTable(listOfRawMaterials[k].raw_material_id,listOfRawMaterials[k].raw_material_name,listOfRawMaterials[k].qty);
	}
    calculateTotal();
  });
...
getRawMaterialsAndStockForThisDate();
