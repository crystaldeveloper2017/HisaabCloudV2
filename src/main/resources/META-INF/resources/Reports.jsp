				...
				<c:forEach items="${message}" var="item">
					...
					<c:forEach var="columnsq1" items="${reportDetails.get('columns')}">
						...
						<%-- <c:choose>
							...
							<c:when test="${fn:contains(columnsq1.get('column_value'), '~delete')}">
								<c:set var="deleteValue" value="${fn:split(columnsq1.get('column_value'), '~')}" />
								<td><a href="?a=${deleteValue[1]}&${item.get(${deleteValue[2]})}">Delete</a></td>
							</c:when>					
							<c:otherwise>
